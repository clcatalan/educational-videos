        ...
        List {
            ForEach(reportList.indices, id: \.self) { index in
                ...
                VStack(alignment: .leading) {
                    ...
                    Text("Start time: \(item.sessionStartTime.dateString)")
                    Text("End time: \(item.sessionEndTime?.dateString ?? "")")
                }
                .listRowBackground(Color(red: 0.38, green: 0.52, blue: 0.68))
                .onTapGesture {
                    print(item) // 선택한 item 로그 찍기
